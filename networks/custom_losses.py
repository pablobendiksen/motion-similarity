...
def calculate_triplet_loss(y_true, y_pred, triplet_mining_module, batch_strategy=BATCH_STRATEGY):
    ...
    classes_distances = triplet_mining_module.calculate_left_right_distances(y_pred)
    triplet_mining_module.calculate_class_neut_distances(y_pred)

    # Reshape the 1D tensor into a 2D tensor with shape (1, 56) (will allow for broadcasting)
    row_dists_class_neut = tf.reshape(triplet_mining_module.tensor_dists_class_neut,
                                      (1, triplet_mining_module.num_states_drives))
    ...
    column_dists_class_neut = tf.reshape(triplet_mining_module.tensor_dists_class_neut,
                                         (triplet_mining_module.num_states_drives, 1))
    ...
    tf.debugging.assert_shapes(
        [(tf.shape(diff_lr_ln), (tf.TensorShape([triplet_mining_module.num_states_drives,
                                                 triplet_mining_module.num_states_drives]),))],
    ...
    if batch_strategy == BatchStrategy.HARD:
        ...
        diff_lr_rl_alpha = diff_lr_ln + triplet_mining_module.matrix_alpha_left_right_right_left
        diff_lr_alpha = tf.multiply(diff_lr_rl_alpha, triplet_mining_module.matrix_bool_left_right)
    ...
    if batch_strategy == BatchStrategy.HARD:
        ...
        diff_rl_rn_alpha = diff_rl_rn + triplet_mining_module.matrix_alpha_left_right_right_left
        diff_rl_alpha = tf.multiply(diff_rl_rn_alpha, triplet_mining_module.matrix_bool_right_left)
    ...
    tf.debugging.assert_shapes([(tf.shape(diff_ln_lr), (tf.TensorShape([triplet_mining_module.num_states_drives,
                                                                        triplet_mining_module.num_states_drives]),))],
                               message="Tensor diff_ln_lr has incorrect shape")
    if batch_strategy == BatchStrategy.HARD:
        diff_ln_lr = tf.where(diff_ln_lr > 0, diff_ln_lr, 0)
        diff_ln_lr_alpha = diff_ln_lr + triplet_mining_module.matrix_alpha_left_neut_neut_left
        # we care only for differences with corresponding alpha_left_neut values (i.e., relevant l_n cases)
        diff_ln_lr_alpha = tf.multiply(diff_ln_lr_alpha, triplet_mining_module.matrix_bool_left_neut)
        # ensure no negative losses
        ...
        diff_ln_lr_alpha = diff_ln_lr + triplet_mining_module.matrix_alpha_right_neut_neut_right
        diff_ln_lr_alpha = tf.multiply(diff_ln_lr_alpha, triplet_mining_module.matrix_bool_left_neut)
        triplet_loss_L_N = tf.where(diff_ln_lr_alpha > 0, diff_ln_lr_alpha, 0)
    # N = anchor
    # diff_nl_nr = column_dists_class_neut - row_dists_class_neut
    diff_nl_nr = row_dists_class_neut - column_dists_class_neut
    tf.debugging.assert_shapes([(tf.shape(diff_nl_nr), (tf.TensorShape([triplet_mining_module.num_states_drives,
                                                                        triplet_mining_module.num_states_drives]),))],
                               message="Tensor diff_nl_nr has incorrect shape")
    if batch_strategy == BatchStrategy.HARD:
        diff_nl_nr = tf.where(diff_nl_nr > 0, diff_nl_nr, 0)
        diff_nl_nr_alpha = diff_nl_nr + triplet_mining_module.matrix_alpha_left_neut_neut_left
        diff_nl_nr_alpha = tf.multiply(diff_nl_nr_alpha, triplet_mining_module.matrix_bool_neut_left)
    ...
    if batch_strategy == BatchStrategy.HARD:
        ...
        diff_r_n_r_l_alpha = diff_rn_rl + triplet_mining_module.matrix_alpha_right_neut_neut_right
        diff_r_n_r_l_alpha = tf.multiply(diff_r_n_r_l_alpha, triplet_mining_module.matrix_bool_right_neut)
    ...
    tf.debugging.assert_equal(triplet_loss_R_N,
                              tf.multiply(triplet_loss_R_N, triplet_mining_module.matrix_bool_right_neut),
                              message="Improper triplet_loss_R_N generation")
    # N = anchor
    diff_nr_nl = column_dists_class_neut - row_dists_class_neut
    tf.debugging.assert_shapes(
        [(diff_nr_nl, (triplet_mining_module.num_states_drives, triplet_mining_module.num_states_drives))],
        message="Tensor diff_nr_nl has incorrect shape")
    if batch_strategy == BatchStrategy.HARD:
        diff_nr_nl = tf.where(diff_nr_nl > 0, diff_nr_nl, 0)
        diff_nr_nl_alpha = diff_nr_nl + triplet_mining_module.matrix_alpha_right_neut_neut_right
        diff_nr_nl_alpha = tf.multiply(diff_nr_nl_alpha, triplet_mining_module.matrix_bool_neut_right)
    ...
    tf.debugging.assert_shapes([(tf.shape(losses), (tf.TensorShape([triplet_mining_module.num_states_drives,
                                                                    triplet_mining_module.num_states_drives]),))],
