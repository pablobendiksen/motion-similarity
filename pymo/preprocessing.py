    ...
    def _to_pos(self, X):
        ...
        for track in X:
            ...
            for joint in track.traverse():
                ...
                rotmats = np.asarray([Rotation([f[0], f[1], f[2]], 'euler','ZYX', from_deg=True).rotmat for f in euler_values])
                ########################################################################################################
                tree_data[joint]=[
    ...
    def _to_expmap(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                rotation_order = r.columns[0][r.columns[0].find('rotation') - 1] + r.columns[1][r.columns[1].find('rotation') - 1] + r.columns[2][r.columns[2].find(
                    'rotation') - 1]  # rotation_order is string : 'XYZ' or'ZYX' or ...
                # funda : added xyz as rotation order
                # rotation_order[::-1]
    ...
    def _expmap_to_euler(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                # funda : added xyz as rotation order
                # rotation_order = r.columns[0][r.columns[0].find('rotation') - 1] + r.columns[1][
                #     r.columns[1].find('rotation') - 1] + r.columns[2][r.columns[2].find(
                #     'rotation') - 1]  # rotation_order is string : 'XYZ' or'ZYX' or ...

                # funda: rotation order is not important here
                # funda: it takes the first group of euler angles
                euler_rots = [Rotation(f, 'expmap', 'XYZ').to_euler(True)[0] for f in expmap] # Convert the eulers to exp maps
...
class ConverterToRightHandedCoordinates(BaseEstimator, TransformerMixin):
    def __init__(self):
        pass
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            # Create a new DataFrame to store the rotated values
            rh_euler_df = pd.DataFrame(index=euler_df.index)
            ...
            rh_euler_df[rxp] = pd.Series(data=-euler_df[rxp], index=euler_df.index)
            rh_euler_df[ryp] = pd.Series(data=euler_df[ryp], index=euler_df.index)
            rh_euler_df[rzp] = pd.Series(data=euler_df[rzp], index=euler_df.index)
            ...
            for joint in joints:
                r = euler_df[[c for c in rots if joint in c]]  # Get the columns that belong to this joint
                rh_euler = [[f[1]['%s_Xrotation' % joint], f[1]['%s_Yrotation' % joint], f[1]['%s_Zrotation' % joint]] for
                         f in r.iterrows()]  # Make sure the columsn are organized in xyz order

                # # Create the corresponding columns in the new DataFrame
                # #
                # rh_euler_rots = [Rotation(f, 'euler', 'XYZ', from_deg=False)._to_rightHand()[0] for f in
                #               rh_euler]  # Convert the left hand to right hand coordinate system

                rh_euler_rots = rh_euler
                # Create the corresponding columns in the new DataFrame

                rh_euler_df['%s_Xrotation' % joint] = pd.Series(data=[e[0] for e in rh_euler_rots], index=rh_euler_df.index)
                rh_euler_df['%s_Yrotation' % joint] = pd.Series(data=[e[1] for e in rh_euler_rots], index=rh_euler_df.index)
                rh_euler_df['%s_Zrotation' % joint] = pd.Series(data=[e[2] for e in rh_euler_rots], index=rh_euler_df.index)

            new_track = track.clone()
            new_track.values = rh_euler_df
            Q.append(new_track)
